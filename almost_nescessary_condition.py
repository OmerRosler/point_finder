...
def almost_nescessary_condition(p : Polynomial):
    ...
    def fn(x,y):
        ...
        k = -m-1-np.floor(np.log(p(y)/p(x))/np.log(y/x))
        ...
        return (cond1 & cond3 & cond_not_trap_exactly & cond2)
    return fn

for p in iterate_possible_polynomials(5, iterate_possible_coefficients()):
    non_diagonal = lambda x,y: x!=y
    non_trivial = lambda x,y : x*y <0.5
    range_of_tp = almost_nescessary_condition(p)

    d = np.linspace(0.5,1,600)
    x,y = np.meshgrid(d,d)
    results = (non_diagonal(x,y) & non_trivial(x,y) & range_of_tp(x,y)).astype(int)
